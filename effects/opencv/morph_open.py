"""
Morphological opening effect using OpenCV.

Opening = erosion followed by dilation. Removes small bright spots (noise).
"""
...
class OpenEffect(BaseUIEffect):
    ...
    def create_control_panel(self, parent):
        ...
        signature_label = ttk.Label(
            header_frame,
            text="cv2.morphologyEx(src, MORPH_OPEN, kernel)",
            font=('TkFixedFont', 12)
        )
    ...
    def draw(self, frame: np.ndarray, face_mask=None) -> np.ndarray:
        """Apply morphological opening to the frame"""
        ...
        # Apply opening
        result = cv2.morphologyEx(frame, cv2.MORPH_OPEN, kernel, iterations=iterations)
