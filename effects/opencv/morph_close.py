"""
Morphological closing effect using OpenCV.
...
class CloseEffect(BaseUIEffect):
    ...
    def create_control_panel(self, parent):
        ...
        if not getattr(self, '_in_pipeline', False):
            ...
            signature_label = ttk.Label(
                header_frame,
                text="cv2.morphologyEx(src, MORPH_CLOSE, kernel)",
                font=('TkFixedFont', 12)
            )
    ...
    def draw(self, frame: np.ndarray, face_mask=None) -> np.ndarray:
        """Apply morphological closing to the frame"""
        ...
        # Apply closing
        result = cv2.morphologyEx(frame, cv2.MORPH_CLOSE, kernel, iterations=iterations)
